...
void Sprite::draw() {
	...
	else
	{
		...
		glm::vec3 p2 = glm::vec3(100, 150, 0);
		glm::vec3 p3 = glm::vec3(-100, 150, 0);
...
bool Sprite::insideTriangle(const glm::vec3 p) {
	...
	cout << verts[0] << endl;
	// for some reason verts is empty here
	...
	*/
	...
	return (glm::distance(p2, localOrigin) < 100);
...
void Sprite::integrate() {
	...
	//for physics based rotation
	rot = rot + rotVelocity * dt;
	rotVelocity = rotVelocity + rotAcceleration * dt;
	rotVelocity = rotVelocity * damping;
